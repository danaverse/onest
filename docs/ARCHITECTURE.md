...
7. **Stamp tiers** — sponsored path plus **client-paid remint** shipped: the desk reserves a non-serving baton (prefers genesis index 26), prepares preimages; the wallet mines PoW on-device, signs and submits; the desk assembles/broadcasts and the minted 108 PAW go to the user's P2PKH output (no desk custody). PWA/mobile detection and Turnstile still to add.
...
- **Client-paid mint (shipped):** when the wallet is short on PAW, the client mints from its own XEC: desk reserves a baton (avoids the desk's serving indices), client mines PoW and signs both preimages with its key, desk verifies/assembles/broadcasts, and the 108 PAW land in the user's wallet. The desk never holds user keys; the desk-mint-and-keep path remains for sponsored stamps. A 1:1 XEC→PAW desk swap is explicitly out of scope for now.
