...
- Memorial tributes keep the sponsored PoW fallback for casual users; posts and votes are XEC-paid whenever a wallet is unlocked, and only visitors without a wallet fall back to the sponsored path until that is retired (see the votes section).
...
- **Wire format:** `DANA` versions — v1/v2 memorial, **v3** vote (`direction`, `targetType`, `postHash`; sender read from the tx), **v4** post stamp (`contentHash`), **v5** memorial carrying the creator's P2PKH `hash160` (on-chain ownership).
...
- **Vote UX:** MVP votes **1 PAW atom at a time**, up or down, from a popup on the vote button (paw icon + up-count). Amount presets and an optional per-tx cap (suggested 108) come after launch. Stacked burns stand: **sum atoms, no one-vote-per-identity lock.**
- **Wallet vote path (shipped):** an unlocked wallet pays a flat XEC fee (`MINT_VOTE_XEC_FEE`, default **6 XEC** = 600 sats ≈ 1–2 XEC network + ≈ 4–5 XEC for the atom) to the desk, which burns **1 PAW with the DANA v3 payload** — no listing fee (nothing is kept), no remint, no PoW, no wait. Same verification as posts/profiles; retries reuse the pending payment per post+direction.
