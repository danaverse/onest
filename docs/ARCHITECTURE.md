...
- Wallet-paid pet profiles: 6 PAW burn (rebirth) + 6-atom desk listing fee, no wait; sponsored root memorials rejected server-side, except the first sponsored pet profile per install (6-atom desk burn, ~2 minute wait, `MINT_PROFILE_MIN_PRAY_SECONDS`). Sponsored post stamps are desk-funded too (1 atom, ~1 minute wait, `MINT_POST_MIN_PRAY_SECONDS`) **only on the install's own pets**; posting on other pets requires a 1 PAW wallet burn or the flat XEC fee
