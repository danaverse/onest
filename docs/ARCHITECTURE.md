...
7. **Stamp tiers** — sponsored path plus **flat-fee paid profiles** shipped: a wallet without PAW pays a flat XEC fee (default 20 XEC, `MINT_PROFILE_XEC_FEE`; ≈75% margin over the burn-tx fee) and the desk burns 1 PAW from inventory. No remint, no PoW, no wait. Sponsored votes always mint fresh, which maintains inventory. PWA/mobile detection and Turnstile still to add.
...
- **Paid profiles (shipped, replaces client self-mint and the order-based exchange):** the user pays a flat XEC fee on-chain (`MINT_PROFILE_XEC_FEE`, default 20 XEC = 2,000 sats; verified on-chain, paid from the creator's wallet, txid single-use) and the desk burns 1 PAW from inventory. No remint, no PoW, no wait; margin ≈ 75% over the burn-tx fee. Sponsored votes keep minting fresh, maintaining inventory. Wallet-paid profiles (1 PAW + 6-atom listing fee) stay for wallets that hold PAW.
