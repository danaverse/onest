...
export interface PowRemintMooreTipMemoParams extends MooreTipParams {
  tokenId: string;
  mintAtoms: bigint;
}

export type PowMooreTipMemoInstance = Instance & { challenges: Challenges };

export interface PowRemintMooreTipMemoContract {
  params: PowRemintMooreTipMemoParams;
  instance: PowMooreTipMemoInstance;
  redeem: EcashScript;
...
export const MOORE_TIP_MEMO_ECON_HEAD_LEN = 85;
export const MOORE_TIP_MEMO_VALUE_OFFSET = 119;
...
async function loadPortable(): Promise<PortableModule> {
  ...
  try {
    const code = readFileSync(
      resolve(process.cwd(), 'contracts/WlotusPowRemintMooreTipMemo.spedn'),
      'utf8',
    );
...
export function findTipValueOffset(
  ...
  if (off !== MOORE_TIP_MEMO_VALUE_OFFSET) {
    throw new Error(`tipValueOffset ${off} != ${MOORE_TIP_MEMO_VALUE_OFFSET}`);
  }
  return off;
}
...
function instantiate(
  ...
  codeHash: Buffer,
  prefixHash: Buffer,
): PowMooreTipMemoInstance {
  const factory = new ModuleFactory(new BchJsRts('mainnet'));
  const Ctor = factory.make(portable).WlotusPowRemintMooreTipMemo;
  return new Ctor(
    ctorArgs(params, codeHash, prefixHash),
  ) as PowMooreTipMemoInstance;
}

export async function createPowRemintMooreTipMemoContract(
  params: PowRemintMooreTipMemoParams,
): Promise<PowRemintMooreTipMemoContract> {
  ...
  if (
    !Buffer.from(
      sha256(redeemScriptBuf.subarray(0, MOORE_TIP_MEMO_ECON_HEAD_LEN)),
    ).equals(prefixHash)
  ) {
...
export async function mooreTipMemoContractForNextTip(
  current: PowRemintMooreTipMemoContract,
  nextTipLocktime: number,
): Promise<PowRemintMooreTipMemoContract> {
  return createPowRemintMooreTipMemoContract({
    ...current.params,
    tipLocktime: nextTipLocktime,
  });
}

export function defaultMooreTipMemoParams(
  tokenId: string,
  genesisUnix: number,
  opts: {
    mintAtoms: bigint;
