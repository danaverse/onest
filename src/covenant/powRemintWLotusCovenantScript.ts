/**
 * WLotusCovenant factory — standard Moore 2× remint covenant (felt +1 bit), ALP MINT only.
 * Smart covenant for Onest (PAW) and WLotus reminting on eCash.
 */
...
export interface PowRemintWLotusCovenantParams extends MooreTipParams {
  tokenId: string;
  mintAtoms: bigint;
}

export type PowRemintMooreTipParams = PowRemintWLotusCovenantParams;
export type PowRemintGlotusTipParams = PowRemintWLotusCovenantParams;

export type PowWLotusCovenantInstance = Instance & { challenges: Challenges };
export type PowMooreTipInstance = PowWLotusCovenantInstance;
export type PowGlotusTipInstance = PowWLotusCovenantInstance;

export interface PowRemintWLotusCovenantContract {
  params: PowRemintWLotusCovenantParams;
  instance: PowWLotusCovenantInstance;
  redeem: EcashScript;
...
export type PowRemintMooreTipContract = PowRemintWLotusCovenantContract;
export type PowRemintGlotusTipContract = PowRemintWLotusCovenantContract;

/** econHead through codeHash; tip opcode at 85+33=118, tip value at 119. */
export const WLOTUS_COVENANT_ECON_HEAD_LEN = 85;
export const GLOTUS_ECON_HEAD_LEN = 85;
export const MOORE_TIP_ECON_HEAD_LEN = 85;
export const WLOTUS_COVENANT_VALUE_OFFSET = 119;
...
export function findTipValueOffset(
  ...
  if (off !== WLOTUS_COVENANT_VALUE_OFFSET) {
    throw new Error(`tipValueOffset ${off} != ${WLOTUS_COVENANT_VALUE_OFFSET}`);
  }
  return off;
}
...
function instantiate(
  ...
  codeHash: Buffer,
  prefixHash: Buffer,
): PowWLotusCovenantInstance {
  const factory = new ModuleFactory(new BchJsRts('mainnet'));
  const Ctor = (factory.make(portable) as any).WLotusCovenant;
  return new Ctor(
    ctorArgs(params, codeHash, prefixHash),
  ) as PowWLotusCovenantInstance;
}

export async function createPowRemintWLotusCovenantContract(
  params: PowRemintWLotusCovenantParams,
): Promise<PowRemintWLotusCovenantContract> {
  ...
  if (
    !Buffer.from(
      sha256(redeemScriptBuf.subarray(0, WLOTUS_COVENANT_ECON_HEAD_LEN)),
    ).equals(prefixHash)
  ) {
...
export const createPowRemintGlotusTipContract = createPowRemintWLotusCovenantContract;
export const createPowRemintMooreTipContract = createPowRemintWLotusCovenantContract;
...
export async function wlotusCovenantContractForNextTip(
  current: PowRemintWLotusCovenantContract,
  nextTipLocktime: number,
): Promise<PowRemintWLotusCovenantContract> {
  return createPowRemintWLotusCovenantContract({
    ...current.params,
    tipLocktime: nextTipLocktime,
  });
}

export const glotusTipContractForNextTip = wlotusCovenantContractForNextTip;
export const mooreTipContractForNextTip = wlotusCovenantContractForNextTip;

export function defaultWLotusCovenantParams(
  tokenId: string,
  genesisUnix: number,
  opts: {
    mintAtoms: bigint;
...
export const defaultGlotusTipParams = defaultWLotusCovenantParams;
export const defaultMooreTipParams = defaultWLotusCovenantParams;
